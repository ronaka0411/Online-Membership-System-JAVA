...
public class EditMember extends JPanel implements ActionListener {
    ...
    private static int countrycnt = 0, statecnt = 0;
    private static String pass;

    public EditMember(String user_id) {
        ...
        try {
            ...
            String response1 = c.sendUserGetRequest(user_id);
            JSONParser parser = new JSONParser();
            Object obj = parser.parse(response1);
            JSONObject json = (JSONObject) obj;
            if (json.containsKey("method") && json.get("method").toString().equalsIgnoreCase("1000")) {
                if (json.containsKey("status") && json.get("status").toString().equalsIgnoreCase("1001")) {
                    username_tt.setText("");
                    ...
                    pass = json.get("password").toString().trim();
                    username_tt.setText(json.get("username").toString().trim());
                    password_tt.setText(pass);
                    firstname_tt.setText(json.get("firstname").toString().trim());
                    lastname_tt.setText(json.get("lastname").toString().trim());
                    address_tt.setText(json.get("address").toString().trim());
                    mobile_tt.setText(Functions.formatPhoneNumber(json.get("phone").toString()));
                    email_tt.setText(json.get("email").toString().trim());

                    statecnt++;
                    countrycnt++;
                    city_tt.removeAllItems();
                    state_tt.removeAllItems();
                    country_tt.removeAllItems();
                    country_tt.addItem(json.get("country").toString().trim());
                    country_tt.setSelectedItem(json.get("country").toString().trim());

                    state_tt.addItem(json.get("state").toString().trim());
                    state_tt.setSelectedItem(json.get("state").toString().trim());

                    city_tt.addItem(json.get("city").toString().trim());

                    city_tt.setSelectedItem(json.get("city").toString().trim());
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().trim().equalsIgnoreCase("Edit")) {
            ...
            } else if (mobile.equalsIgnoreCase("") || !Functions.validatePhoneNumber(mobile)) {
                errormsg = "Enter Valid Mobile Number";
                JOptionPane.showMessageDialog(null, errormsg);
            } else {


                if (!pass.toString().trim().equals(password.toString().trim())) {
                    password = Functions.MD5(password);
                }
                pass = "";
                try {
                    Client c = new Client();
                    String response = c.sendUserEditData(uid, username, password, firstname, lastname, country, state, city, address, mobile, email);
                    JSONParser parser = new JSONParser();
                    Object obj = parser.parse(response);
                    JSONObject json = (JSONObject) obj;
    ...
    public static void open(String user_id) {
        uid = user_id;
        frame = new JFrame("Edit Member : " + Functions.getAppName());
        frame.setResizable(false);
        // frame.setUndecorated(true);

        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.getContentPane().add(new EditMember(user_id));
